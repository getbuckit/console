...
swagger-console:
	...
	@npx swagger-typescript-api generate -p ./swagger.yml -o ./web-app/src/api -n consoleApi.ts --custom-config generator.config.js
...
coverage:
	@(GO111MODULE=on go test -v -coverprofile=coverage.out github.com/getbuckit/console/api/... && go tool cover -html=coverage.out && open coverage.html)
